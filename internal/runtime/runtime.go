...
func (r *Runner) Run(ctx context.Context, once bool) error {
	if err := r.Reconciler.Bootstrap(ctx, r.Plan); err != nil {
		return err
	}
	r.log(Event{Event: "velvet-bootstrap", Status: "success", NodeUID: r.Plan.UUID.String()})
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	r.mu.Lock()
	r.states = make(map[string]materializedState)
	r.mu.Unlock()
	established := make(chan struct{}, len(r.Plan.Links))
	for i := range r.Plan.Links {
		desired := r.Plan.Links[i]
		go r.runLink(ctx, desired, established)
	}
	if once {
		for range r.Plan.Links {
			select {
			case <-established:
			case <-ctx.Done():
				return ctx.Err()
...
func (r *Runner) dialAndServe(ctx context.Context, desired reconcile.LinkPlan, once *sync.Once, established chan<- struct{}) error {
	remote := &net.TCPAddr{IP: net.IP(desired.BootstrapPeer.AsSlice()), Port: r.Plan.VFPPort, Zone: desired.InterfaceName}
...
func (r *Runner) listenAndServe(ctx context.Context, desired reconcile.LinkPlan, once *sync.Once, established chan<- struct{}) error {
	address := &net.TCPAddr{IP: net.IP(desired.BootstrapAddress.Addr().AsSlice()), Port: r.Plan.VFPPort, Zone: desired.InterfaceName}
...
func (r *Runner) serveSession(ctx context.Context, conn net.Conn, desired reconcile.LinkPlan, once *sync.Once, established chan<- struct{}) error {
	localUID := message.UID{UUID: r.Plan.UUID, Name: r.Plan.UID.Name}
	protocol := engine.New(engine.Config{
		LocalUID:        localUID,
		LocalLoopbackV6: r.Plan.LoopbackV6,
		LinkPoolV4:      r.Plan.LinkPoolV4, LinkPoolV6: r.Plan.LinkPoolV6,
		LoopbackPoolV6: r.Plan.LoopbackPoolV6,
		FabricPSK:      r.Plan.FabricPSK, LinkOverrides: desired.LinkOverrides,
		Accept: func(remote message.UID, proposal link.Proposal) bool {
			if !link.MatchesOverrides(proposal, desired.LinkOverrides) {
				return false
			}
			localAddresses, _ := link.EndpointAddresses(proposal, r.Plan.UUID, remote.UUID)
			return r.Reconciler.ProposalAvailable(ctx, desired, proposal, localAddresses)
		},
		Commit: func(result engine.Result) error {
			localAddresses, _ := link.EndpointAddresses(result.Proposal, r.Plan.UUID, result.RemoteUID.UUID)
			peerLoopbacks := validAddresses(result.RemoteLoopbackV6)
			if err := r.Reconciler.Materialize(ctx, desired, localAddresses, peerLoopbacks); err != nil {
				return err
			}
			r.mu.Lock()
			...
			r.mu.Unlock()
			r.log(Event{Event: "velvet-link-established", Status: "success", NodeUID: r.Plan.UUID.String(), Peer: desired.PeerName, Interface: desired.InterfaceName, RemoteUID: result.RemoteUID.UUID.String()})
...
func (r *Runner) maintain(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			if err := r.Reconciler.MaintainBootstrap(ctx, r.Plan); err != nil {
				r.log(Event{Event: "velvet-reconcile", Status: "failed", NodeUID: r.Plan.UUID.String(), Error: err.Error()})
				continue
			}
			r.mu.RLock()
			...
			for _, state := range states {
				if err := r.Reconciler.Materialize(ctx, state.desired, state.local, state.peers); err != nil {
